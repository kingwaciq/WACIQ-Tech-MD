    ...
    try {
        ...
        try {
            ...
            if (!adminStatus.isBotAdmin) {
                await sock.sendMessage(chatId, {
                    text: '🔒 *Arslan-Tech-MD* needs admin powers to swing the demotion hammer! 🔨'
                });
                return;
            }
            ...
            await sock.sendMessage(chatId, {
                text: '⚠️ Arslan-Tech-MD can’t check admin status. Make sure the bot has permissions.'
            });
            return;
        }
        ...
        const demotionMessage =
            `💔 *ARSLAN-TECH-MD - DEMOTION NOTICE* 💔\n\n` +
        ...
        if (error.data === 429) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '⚠️ Rate limit reached! Let Arslan-Tech-MD catch its breath. 😮‍💨'
                });
            } catch (retryError) {
            ...
            try {
                await sock.sendMessage(chatId, {
                    text: '❌ Arslan-Tech-MD failed to demote! Is the user already powerless or bot not admin?'
                });
            } catch (sendError) {
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `💢 *AUTO-DEMOTION DETECTED BY ARSLAN-TECH-MD* 💢\n\n` +
